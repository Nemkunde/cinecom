...
export const movieDbCall = (single: boolean, movieId?: number) => {
    const baseQuery = db
        ...
        .from(moviesTable)
        .leftJoin(movieGenresTable, eq(movieGenresTable.movie_id, moviesTable.movie_id))
        .leftJoin(genresTable, eq(genresTable.genre_id, movieGenresTable.genre_id))
        .leftJoin(movieActorsTable, eq(movieActorsTable.movie_id, moviesTable.movie_id))
        .leftJoin(actorsTable, eq(actorsTable.actor_id, movieActorsTable.actor_id))
        .leftJoin(movieDirectorsTable, eq(movieDirectorsTable.movie_id, moviesTable.movie_id))
        .leftJoin(directorsTable, eq(directorsTable.director_id, movieDirectorsTable.director_id));
